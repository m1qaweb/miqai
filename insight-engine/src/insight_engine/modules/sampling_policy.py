...
class FixedRateSamplingPolicy(SamplingPolicy):
    ...
    def __init__(self, rate_fps: int):
        ...
        logger.info(
            f"Initialized FixedRateSamplingPolicy with rate: {rate_fps} FPS (interval: {self.interval_seconds:.2f}s)"
        )
    ...
    def should_process(self, features: Dict[str, Any]) -> bool:
        ...
        if current_time is None:
            logger.warning(
                "Timestamp not found in features. Defaulting to processing frame."
            )
            return True

        if (
            self.last_processed_time < 0
            or (current_time - self.last_processed_time) >= self.interval_seconds
        ):
...
class HeuristicSamplingPolicy(SamplingPolicy):
    ...
    def __init__(self, motion_threshold: float = 0.1):
        ...
        logger.info(
            f"Initialized HeuristicSamplingPolicy with motion threshold: {self.motion_threshold}"
        )
    ...
    def should_process(self, features: Dict[str, Any]) -> bool:
        ...
        if motion_magnitude > self.motion_threshold:
            logger.debug(
                f"Motion ({motion_magnitude:.2f}) exceeded threshold ({self.motion_threshold}). Processing frame."
            )
            return True
        return False
...
class LearnedSamplingPolicy(SamplingPolicy):
    ...
    def _warm_up_models(self):
        ...
        try:
            ...
            dummy_input = np.random.rand(
                *[1 if s == "N" or not isinstance(s, int) else s for s in input_shape]
            ).astype(np.float32)
    ...
    def _has_significant_semantic_change(self, features: Dict[str, Any]) -> bool:
        ...
        current_embedding = self.feature_extractor_session.run(
            None, {extractor_input_name: current_frame}
        )[0]
        ...
        policy_input = np.concatenate(
            [embedding_diff.flatten(), [time_since_last]]
        ).astype(np.float32)
        ...
        decision_logit = self.policy_session.run(
            None, {policy_input_name: policy_input}
        )[0]
    ...
    def should_process(self, features: Dict[str, Any]) -> bool:
        ...
        if self._has_significant_motion(features):
            ...
            self.last_processed_embedding = (
                None  # Invalidate to force re-compute on next semantic check
            )
            return True
        ...
        if (
            self.last_processed_time < 0
            or (current_time - self.last_processed_time) >= self.time_threshold_seconds
        ):

...
class AccuracyMonitor:
    ...
    def is_breached(self) -> bool:
        ...
        if (
            self.baseline_avg < 0
            or len(self.candidate_metrics) < self.candidate_metrics.maxlen / 2
        ):
            return False
...
class SafetyGuard(SamplingPolicy):
    ...
    def __init__(
        ...
        logger.info(
            f"SafetyGuard initialized for policy {type(primary_policy).__name__}."
        )
    ...
    def _try_recover(self):
        ...
        if time.time() - self.fallback_activation_time >= self.cooldown_seconds:
            logger.info(
                "Cooldown period ended. Attempting to recover to primary policy."
            )
            self.is_fallback_active = False
            self.fallback_activation_time = -1.0
    ...
    def should_process(self, features: Dict[str, Any]) -> bool:
        ...
        if self.is_fallback_active:
            recovered_this_call = (
                self.fallback_activation_time > 0
                and (time.time() - self.fallback_activation_time)

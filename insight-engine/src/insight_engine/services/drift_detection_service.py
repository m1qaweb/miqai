...
class DriftAlert(BaseModel):
    ...
    model_name: str = Field(
        ..., description="The name of the model experiencing drift."
    )
    detected_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    drift_score: float = Field(
        ..., description="The calculated drift score (e.g., KL divergence)."
    )
    threshold: float = Field(..., description="The drift threshold that was breached.")
    comparison_window_start: datetime = Field(
        ..., description="Start of the comparison data window."
    )
    comparison_window_end: datetime = Field(
        ..., description="End of the comparison data window."
    )


# Prometheus Metrics defined as per drift_detection_design.md
DRIFT_SCORE = Gauge("video_ai_drift_score", "Latest drift score.", ["method", "source"])
DRIFT_CHECK_DURATION = Histogram(
    "video_ai_drift_check_duration_seconds", "Latency of the drift check operation."
)
DRIFT_DETECTED_TOTAL = Counter(
    "video_ai_drift_detected_total", "Total number of times drift has been detected."
)
...
class DriftDetectionService:
    ...
    def _calculate_kl_divergence(
        self, p_samples: np.ndarray, q_samples: np.ndarray
    ) -> float:
        """
    ...
    def check_drift(
        ...
        if (
            ref_embeddings.shape[0] < self.pca.n_components
            or comp_embeddings.shape[0] < self.pca.n_components
        ):
        ...
        kl_divergence = self._calculate_kl_divergence(
            ref_embeddings_pca, comp_embeddings_pca
        )
        logger.info(f"Calculated KL Divergence: {kl_divergence}")

        # 4. Update Prometheus Gauge
        DRIFT_SCORE.labels(method="kl_divergence", source="embeddings").set(
            kl_divergence
        )
        ...
        if drift_detected:
            logger.warning(
                f"Drift detected! Score: {kl_divergence} > Threshold: {self.drift_threshold}"
            )
            DRIFT_DETECTED_TOTAL.inc()

        return {"drift_detected": drift_detected, "kl_divergence": kl_divergence}

    async def trigger_retraining(
        self,
        event_id: str,
        comparison_window_start: float,
        ...
        if not self.retraining_webhook_url:
            ...
            return {
                "status": "failed",
                "reason": "Retraining webhook URL not configured.",
            }
        ...
        logger.info(
            f"Packaging data for event {event_id} from {comparison_window_start} to {comparison_window_end}"
        )

        # Placeholder for fetching representative data and uploading to S3
        s3_data_path = (
            f"s3://video-ai-retraining-data/batch-{event_id}-{int(time.time())}.zip"
        )
        ...
        try:
            logger.info(
                f"Triggering retraining pipeline at {self.retraining_webhook_url}"
            )
            response = await self.http_client.post(
                self.retraining_webhook_url, json=payload, timeout=30.0
            )
            response.raise_for_status()

            retraining_job_id = response.json().get("retraining_job_id", "unknown")
            logger.info(
                f"Successfully triggered retraining. Job ID: {retraining_job_id}"
            )
            ...
            logger.error(
                f"Failed to trigger retraining pipeline for event {event_id}: {e}"
            )

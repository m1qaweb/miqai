...
class ModelVersion(BaseModel):
    ...
    stage: str = Field(
        ..., description="The deployment stage (e.g., Staging, Production)."
    )
    description: Optional[str] = Field(
        None, description="A description of the model version."
    )
...
class ModelRegistryService:
    ...
    def _write_registry(self, data: Dict):
        ...
        try:
            ...
            logger.error(
                f"Could not acquire lock on {self.registry_path}. Operation failed."
            )
            raise

    def _get_next_version(self, models: List[Dict], model_name: str) -> int:
        """Determines the next version number for a given model."""
        versions = [m["version"] for m in models if m.get("model_name") == model_name]

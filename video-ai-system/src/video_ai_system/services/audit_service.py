        ...
        async with self._lock:
            ...
            if last_line:
                try:
                    ...
                    return last_log_data.get("entry_hash", self._calculate_hash("GENESIS_BLOCK"))
                except json.JSONDecodeError:
                    logger.error("Could not parse last line of audit log. Returning genesis hash.")
                    return self._calculate_hash("GENESIS_BLOCK")
            return self._calculate_hash("GENESIS_BLOCK")

    async def log(self, actor: str, action: str, details: Dict[str, Any]) -> AuditLogEntry:
        """

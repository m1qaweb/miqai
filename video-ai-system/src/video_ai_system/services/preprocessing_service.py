...
class VideoPreprocessor:
    ...
    def __init__(self, config: dict):
        ...
        logger.info(f"VideoPreprocessor initialized with strategy: {self.hash_algorithm}")
    ...
    def extract_keyframes(
        ...
        try:
            while cap.isOpened():
                ...
                if last_keyframe_hash is None or (current_hash - last_keyframe_hash) > self.threshold:
                    logger.debug(
            ...
            cap.release()
            logger.info(f"Finished processing {video_path}. Analyzed {frame_count} frames.")
    ...
    async def process_video(self, video_path: str) -> List[np.ndarray]:
        ...
        logger.info(f"Extracted a total of {len(keyframes)} keyframes from {video_path}")
        return keyframes
    ...
    async def process_video_with_frame_numbers(self, video_path: str) -> List[Dict]:
        ...
        logger.info(f"Extracted a total of {len(keyframes_with_info)} keyframes with info from {video_path}")
        return keyframes_with_info
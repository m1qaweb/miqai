...
class DriftDetectionModule(VideoModule):
    ...
    def process(self, embedding: np.ndarray) -> np.ndarray:
        ...
        if embedding_norm == 0 or baseline_norm == 0:
            ...
            cosine_similarity = np.dot(embedding, self.baseline_embedding) / (embedding_norm * baseline_norm)
        
        # Distance is 1 - similarity
        distance = 1.0 - cosine_similarity
        
        if distance > self.drift_threshold:
            print(f"WARNING: Drift detected! Distance: {distance:.4f} > Threshold: {self.drift_threshold}")

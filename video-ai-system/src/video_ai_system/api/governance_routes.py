...
@router.get("/shadow_results/{model_name}/{model_version}", response_model=ShadowTestResult)
...
async def trigger_retraining(
    ...
    logger.info(f"Triggering retraining for model '{alert.model_name}' due to alert '{alert_id}'.")
...
async def decide_on_rollout(
    ...
    return {"message": f"Decision for model {decision.model_name} v{decision.model_version} has been recorded."}
